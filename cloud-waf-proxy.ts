...
const otpSchema =
    new mongoose.Schema<IOTPVerification>({
         ...
         * عملية إنشاء الحساب.
         *
         * لا يتم حفظ كلمة المرور الأصلية.
         */
        passwordHash: {
            type: String,
            required: true
...
const OTPModel =
    (mongoose.models.OTPVerification as
        | Model<IOTPVerification>
        | undefined) ||
...
let mongoConnectionStarted = false;

async function connectAuthenticationDatabase(): Promise<void> {
    if (
        mongoose.connection.readyState === 1 ||
        mongoConnectionStarted
    ) {
        return;
    }

    mongoConnectionStarted = true;
...
/* =========================================================
   ...
   =========================================================
   هذا الجزء لا ينشئ Express Server جديد.
   يتم تمرير app الموجود في cloud-waf-proxy.ts.
========================================================= */

export function setupRoutixAuthentication(
    app: Express
): void {
    ...
    app.post(
        ...
        async (
            ...
            try {
                ...
                await connectAuthenticationDatabase();
...
`
    ...
    /* =====================================================
       ...
       ملاحظة:
       إذا كان cloud-waf-proxy.ts لديه Global Error Handler
       خاص به، لا تضف Handler آخر بعد هذا الجزء.
    ===================================================== */
...
/* =========================================================
   ...
   لأن cloud-waf-proxy.ts هو الذي يشغّل السيرفر
   ويستمع على PORT الخاص بـ Render.
